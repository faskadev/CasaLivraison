...
## 📱 Génération de l'APK Android (Production)

Pour générer un fichier APK installable (nécessite un compte Expo) :
...
3. Lancer la compilation pour Android :
   ```bash
   eas build -p android --profile preview
   ```

4. Téléchargez le fichier `.apk` généré depuis le lien fourni par Expo.
...
npm test
```
(Assurez-vous que la base de données de test est accessible ou configurée).

## 📋 Fonctionnalités Clés
- **Authentification:** Inscription et Connexion (Client, Livreur).
- **Catalogue:** Navigation par catégorie (Marocaine, Asiatique, Street Food).
- **Commande:** Ajout au panier, validation de commande.
- **Suivi:** Consultation de l'historique et statut des commandes.
